...
/// Pre-created wgpu objects needed to construct a Renderer.
/// Platform-specific code is responsible for creating these.
...
impl Renderer {
    /// Create a new Renderer from pre-created wgpu resources.
    ///
    /// Platform-specific code should create the `RendererResources` (surface, device, queue, config)
    /// and pass them here. This avoids duplicating GPU setup across platforms.
    pub fn new(resources: RendererResources, msaa_sample_count: u32) -> Self {
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            contents: bytemuck::cast_slice(&vec![
                FiberVertex::new(glam::Vec2::ZERO, [0.0; 4]);
                MAX_VERTICES
            ]),
...
/// Helper to create adapter, device, queue, and configure a surface.
/// The caller must create the wgpu Instance and Surface (platform-specific),
/// then call this to complete setup.
pub async fn setup_wgpu_device_and_config(
